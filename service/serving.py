...
from conf.settings import FilesConf, ModelConf
from service.data_preparation import get_data, DataSets
from service.ml_models import regression_map
from numpy.random import choice
from service.logging import results_as_dict
...
with open(FilesConf.FileNames.runtime) as file:
...
def train_precedure():
    try:
        ...
        for p in range(1, 13):
            ...
            print("------------ Delta: " + str(p))
            res, reg_conf = optimize_hyperparameters(model_key, ModelConf.random_search, datasets, temporal_validation)  # [test_cols])
        ...
        with open(FilesConf.FileNames.train_results, "w") as file:
...
def production_model_procedure():
    try:
        model_periods = {}
        model_results = {}
        with open(FilesConf.FileNames.train_results) as file:
        ...
        for p in range(1, 13):
            ...
            with open(FilesConf.Paths.model + "/" + str(p), "w") as file:
                    file.write(pickle.dumps(pres.model).hex())
            model_results[p] = {
                "config": reg_conf,
                "general": results_as_dict(pres),
            ...
            del dts
            del pres
            del temporal_validation
            del lags
            del comp_data
            del reg_conf

        del model_results_training
        with open(FilesConf.FileNames.production_model, "w") as file:
            file.write(json.dumps(model_results, cls=GeneralJsonEncoder))
        return True
    except Exception as e:
        print(e)
...
def prediction_precedure():
    def try_or_fail(): 
    #try:
        with open(FilesConf.FileNames.production_model) as file:
        ...
        print("Database len: " + str(database_df.shape))
        year  = int(database_df.year.max())
        month = int(database_df.query("year == " + str(year)).month.max())
        ...
        for p in range(1, 13):
            ...
            print(str(p) + " preduction data: " + str(data.shape))
            print(str(p) + " loading model...")
            with open(FilesConf.Paths.model + "/" + str(p)) as file:
                model = pickle.loads(bytes.fromhex(file.read()))
            cols = production_model[model_index]["cols"]
            survive = ["time","year", "month", "age_range", "economic_division", "gender", "value"]
            print(str(p) + " loading dataset...")
            dts = DataSets(data,
                                encode_string=False,
                                one_hot_encode=False,
                                categ_to_num=True,
                                ...
                                transformations=variables.get("transformations"),
                                shuffle=True)
            print(str(p) + " generating predictions...")
            res = model.predict(dts.external(data[cols])[cols])
            data["value"] = dts.inverse_function[dts.link](res)
            print(str(p) + " concat predictions...")
            prediction = pd.concat([prediction, data[survive]])
            #del data
            del dts
            del model

        prediction.to_csv(FilesConf.Paths.output + "/{}_prediction.csv".format(datetime.datetime.now().strftime("%Y%m%d%H%M%S")))
        return True
    return try_or_fail()
    #except Exception as e:
    #    print(e)
